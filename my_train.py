...
from medmnist.my_model import ResNet18, ResNet50
...
def main(flag, input_root, output_root, end_epoch, download, resnet):
    ''' main function
    ...
    batch_size = 128
    val_auc_list = []
    dir_path = os.path.join(output_root, 'my_resnet%s_result/%s_checkpoints' % (resnet, flag))
    ...
    epochs = range(1, end_epoch + 1)
    plt.plot(epochs, val_auc_list, 'b', label='Training_accuracy')
    plt.title('Training Accuracy with %s Epochs' % (end_epoch))
    plt.xlabel('Epochs')
    plt.ylabel('Auc')
    plt.legend()
    ...
    restore_fig_path = os.path.join(
        output_root, 'my_resnet%s_result/%s/%s_training_curve.png' % (resnet, flag, flag))
    plt.savefig(restore_fig_path)
...
def test(model, split, data_loader, device, flag, task, output_root=None):
    ...
    with torch.no_grad():
        ...
        if output_root is not None:
            output_dir = os.path.join(output_root,'my_resnet%s_result/%s' % (resnet, flag))
